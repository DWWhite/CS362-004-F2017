...
int main() {
    ...
    //add a smithy to the hand
    G.hand[turn][G.handCount[turn]]=smithy;
    G.handCount[0]=G.handCount[0]+1;           //add handcount
    int handPos= G.handCount[turn]-1;             //get position of smithy in hand
    ...
    for(int turn=0;turn<numPlayer;turn++){
        ...
        if(G.whoseTurn==turn){
            ...
            if(G.handCount[turn]!= Backup.handCount[turn]+3-1){
                printf("\n****Error- Hand Count is off  should be :%d Actually is: %d\n", Backup.handCount[turn]+3-1, G.handCount[turn]);
                count++;
            }
        }
        else{
